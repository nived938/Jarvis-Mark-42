...
- embedded Google Maps JavaScript map in the center HUD
- Places-powered text search
- Places autocomplete inside the map
- place markers for search results
- place details on marker selection when the configured key permits them
- local 127.0.0.1 page bridge so the Google Maps client loads from a normal web origin
- API key loaded only from local config/api_keys.json
- no assumption of Routes, Street View, or other APIs not granted to the key
