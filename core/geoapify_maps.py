...
def _api_key() -> str:
    ...
    if not key or key == "YOUR_NEW_KEY":
        raise RuntimeError(
            "Geoapify is not configured. Put geoapify_api_key in "
...
def _request_json(url: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError("Geoapify returned invalid JSON.") from exc
            ...
            results.append(
                {
                    ...
                    or "Location",
                    "formatted": item.get("formatted") or item.get("address_line2") or "",
                    "lat": float(item["lat"]),
                    "lon": float(item["lon"]),
                    "place_id": item.get("place_id"),
                    "type": item.get("result_type") or "",
                }
            )
...
def search(query: str, lat: float, lon: float) -> dict[str, Any]:
    ...
    if category:
        ...
        if cleaned:
            location_hits = geocode(cleaned, limit=1)
            if location_hits:
                lat, lon = location_hits[0]["lat"], location_hits[0]["lon"]
        return {"kind": "places", "query": text, "results": places(category, lat, lon)}

    return {"kind": "geocode", "query": text, "results": geocode(text, limit=8)}


def _html(api_route_prefix: str = "") -> str:
    # API key is never embedded in this page. Browser requests stay on localhost,
    # and the Python bridge talks to Geoapify with the key loaded from config.
...
.leaflet-popup-content a{{color:#00d4ff;}}
#status{{position:fixed;left:12px;top:12px;z-index:9999;padding:7px 10px;border:1px solid #12617a;
background:rgba(1,10,16,.9);color:#8ffcff;font:11px Consolas,monospace;border-radius:5px;}}
</style>
</head>
<body>
<div id="map"></div>
<div id="status">GEOAPIFY MAP READY</div>
<script src="{_LEAFLET_JS}"></script>
<script>
const map = L.map('map', {{zoomControl:true}}).setView([20, 78], 5);
L.tileLayer('/tiles/carto/{{z}}/{{x}}/{{y}}.png', {{
    maxZoom: 19,
    attribution: 'Powered by <a href="https://www.geoapify.com/" target="_blank">Geoapify</a> | © <a href="https://www.openstreetmap.org/copyright">OpenStreetMap contributors</a>'
}}).addTo(map);
...
function showResults(payload) {{
  ...
  rows.forEach((p, idx) => {{
    ...
    const title = esc(p.name || 'Location');
    const address = esc(p.formatted || '');
    marker.bindPopup('<b>'+title+'</b><br>'+address);
    bounds.push([p.lat,p.lon]);
  }});
  map.fitBounds(bounds, {{padding:[36,36], maxZoom:16}});
  document.getElementById('status').textContent =
    (payload.kind === 'places' ? 'PLACES' : 'SEARCH') + ' • ' + rows.length + ' RESULT(S)';
}}
...
async function searchMap(q) {{
  ...
  try {{
    const r = await fetch('/api/search?'+new URLSearchParams({{q:q,lat:c.lat,lon:c.lng}}));
    const payload = await r.json();
    if (!r.ok) throw new Error(payload.error || 'Search failed');
    showResults(payload);
  }} catch (e) {{
...
async function routeTo(q) {{
  document.getElementById('status').textContent = 'FINDING DESTINATION…';
  try {{
    const current = map.getCenter();
    const g = await fetch('/api/geocode?'+new URLSearchParams({{text:q}}));
    const gp = await g.json();
    if (!g.ok || !(gp.results||[]).length) throw new Error('Destination not found');
    const d = gp.results[0];
    const r = await fetch('/api/route?'+new URLSearchParams({{
      slat:current.lat, slon:current.lng, elat:d.lat, elon:d.lon, mode:'drive'
    }}));
    const payload = await r.json();
    if (!r.ok) throw new Error(payload.error || 'Route failed');
    if (routeLayer) map.removeLayer(routeLayer);
    routeLayer = L.geoJSON(payload, {{
      style: {{color:'#00d4ff',weight:5,opacity:.85}}
    }}).addTo(map);
    map.fitBounds(routeLayer.getBounds(), {{padding:[30,30]}});
    document.getElementById('status').textContent = 'ROUTE • DRIVE';
  }} catch (e) {{
    document.getElementById('status').textContent = 'ROUTE ERROR: ' + e.message;
  }}
}}
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if path.startswith("/tiles/carto/") and path.endswith(".png"):
                ...
                if len(parts) != 5:
                    self._json(404, {"error": "Invalid tile path"})
                    return
                z, x, filename = parts[2], parts[3], parts[4]
                y = filename[:-4]
                key = _api_key()
                upstream = (
                    f"https://maps.geoapify.com/v1/tile/carto/{z}/{x}/{y}.png"
                    f"?apiKey={urllib.parse.quote(key)}"
                )
                data, content_type = _request_bytes(upstream)
                self._send(200, data, content_type)
                return

            if path == "/api/geocode":
                text = (query.get("text") or [""])[0]
                self._json(200, {"results": geocode(text)})
                return

            if path == "/api/search":
                text = (query.get("q") or [""])[0]
                lat = float((query.get("lat") or ["20"])[0])
                lon = float((query.get("lon") or ["78"])[0])
                self._json(200, search(text, lat, lon))
                return

            if path == "/api/route":
                result = route(
                    float((query.get("slat") or ["0"])[0]),
                    float((query.get("slon") or ["0"])[0]),
                    float((query.get("elat") or ["0"])[0]),
                    float((query.get("elon") or ["0"])[0]),
                    str((query.get("mode") or ["drive"])[0]),
                )
...
def map_url(query: str = "") -> str:
    ...
    if q:
        return f"http://127.0.0.1:{port}/?q={urllib.parse.quote(q)}"
