...
def _validate_generated_code(path: Path
code: str) -> tuple[bool
str]:
...
return False
(proc.stderr or proc.stdout or "JavaScript syntax check failed.").strip()[:2000]
return True
...
return True
"No language-specific validator available; file accepted.",,,def _edit_action(file_path
instruction
player
...
4):
if attempt == 1:
prompt = f"""You are an expert code editor.,Apply the following change to the code below.,Return ONLY the complete updated code. No explanation or markdown.,,Change: {instruction},,Original code:,{original},,Updated code:"""
else:
prompt = f"""Repair the following code edit.,Preserve the requested change and return ONLY the complete corrected code.,,Requested change: {instruction},Validation error: {last_error[:2000]},,Current code:,{edited},"""
try:
response = model.generate_content(prompt)
...
break,
ok
detail = _validate_generated_code(path
edited)
if ok:
break,
last_error = detail
if player:
player.write_log(f"[Code] Validation failed
repairing edit ({attempt}/3)...")
if attempt == 3:
...
previous = original
def _undo_edit(p=path
old=previous):
p.write_text(old
encoding="utf-8")
return f"Restored {p.name} to its previous contents.",
push_undo(f"edited {path.name}"
_undo_edit),
validation = ""
if verify:
_ok
