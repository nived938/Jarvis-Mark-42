...
def _validate_generated_code(path: Path, code: str) -> tuple[bool, str]:,    """Validate generated source with a cheap language-specific check.""",    suffix = path.suffix.lower(),    try:,        if suffix == ".py":,            compile(code, str(path), "exec"),            return True, "Python syntax OK.",        if suffix == ".json":,            json.loads(code),            return True, "JSON syntax OK.",        if suffix in (".js", ".mjs", ".cjs") and shutil.which("node"):,            import tempfile,            with tempfile.NamedTemporaryFile("w", suffix=suffix, delete=False, encoding="utf-8") as f:,                tmp = Path(f.name),                f.write(code),            try:,                proc = subprocess.run(["node", "--check", str(tmp)], capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=15),            finally:,                tmp.unlink(missing_ok=True),            if proc.returncode != 0:,                return False, (proc.stderr or proc.stdout or "JavaScript syntax check failed.").strip()[:2000],            return True, "JavaScript syntax OK.",    except SyntaxError as e:,        return False, f"Syntax error: {e}",    except json.JSONDecodeError as e:,        return False, f"JSON syntax error: {e}",    except Exception as e:,        return False, f"Validation error: {e}",    return True, "No language-specific validator available; file accepted.",,,def _edit_action(file_path, instruction, player, verify: bool = True) -> str:,    if not file_path:,        return "Please provide a file path to edit, sir.",    if not instruction:,        return "Please describe what change to make, sir.",,    path = Path(file_path),    original, err = _read_file(file_path),    if err:,        return err,,    if player:,        player.write_log("[Code] Editing file..."),,    model = _get_gemini(),    edited = "",    last_error = "",,    for attempt in range(1, 4):,        if attempt == 1:,            prompt = f"""You are an expert code editor.,Apply the following change to the code below.,Return ONLY the complete updated code. No explanation or markdown.,,Change: {instruction},,Original code:,{original},,Updated code:""",        else:,            prompt = f"""Repair the following code edit.,Preserve the requested change and return ONLY the complete corrected code.,,Requested change: {instruction},Validation error: {last_error[:2000]},,Current code:,{edited},""",        try:,            response = model.generate_content(prompt),            edited = _clean_code(response.text),        except Exception as e:,            return f"Could not edit code: {e}",,        if not verify:,            break,,        ok, detail = _validate_generated_code(path, edited),        if ok:,            break,,        last_error = detail,        if player:,            player.write_log(f"[Code] Validation failed, repairing edit ({attempt}/3)..."),        if attempt == 3:,            return f"Edit rejected because validation kept failing: {detail}",,    try:,        path.parent.mkdir(parents=True, exist_ok=True),        path.write_text(edited, encoding="utf-8"),    except Exception as e:,        return f"Could not save edited file: {e}",,    previous = original,    def _undo_edit(p=path, old=previous):,        p.write_text(old, encoding="utf-8"),        return f"Restored {p.name} to its previous contents.",,    push_undo(f"edited {path.name}", _undo_edit),,    validation = "",    if verify:,        _ok, detail = _validate_generated_code(path, edited),        validation = f"\n\nValidation: {detail}",,    print(f"[Code] ✅ Edited: {file_path}"),    return f"File edited. Saved to: {path}{validation}\n\nPreview:\n{_preview(edited)}",,,
